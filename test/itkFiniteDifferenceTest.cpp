...
int df_test_metricv3(){
    ...
    for(unsigned i = 0; i<100; i++){
        ...
        if(i==1){
            ...
            metric->GetValueAndDerivative(pz, val_r, deriv_r);
        ...
        unsigned idx = dis_int(gen);
        {
            ...
            MetricType::MeasureType val_h = metric->GetValue(pz);
...
int main (int argc, char * argv[]) {  
    #if SinglePrecisionType==0
    {
        std::cout << "***** Testing Metric v3 *****" << std::endl;
        auto t0 = std::chrono::system_clock::now();
        df_test_metricv3();
        std::chrono::duration<double> elapsed_seconds = std::chrono::system_clock::now()-t0;
